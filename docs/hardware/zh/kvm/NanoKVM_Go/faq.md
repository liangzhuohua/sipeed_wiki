...
### 使用功率不足的电源适配器或不支持 PD 快充的充电线导致 NanoKVM Go 反复重启

NanoKVM Go 的供电方式如下：

- 如果不连接**辅助 USB-C 接口（闪电图标，Power Port）**，NanoKVM Go 由**被控设备的 USB-C 接口供电**；
- 如果连接了**辅助 USB-C 接口（闪电图标，Power Port）**，接入该接口的电源适配器需要**同时为被控设备和 NanoKVM Go 供电**（该接口支持 USB-PD 充电直通，可继续为被控设备充电）。

因此，当用户将**功率不足的电源适配器**（如旧式 5V/1A 电源适配器）或**不支持 USB-PD 快充协议**的充电线接入辅助 USB-C 接口时，输入端无法同时满足被控设备和 NanoKVM Go 的供电需求，NanoKVM Go 会因为供电不足而出现**不断重启**的现象。

**解决办法：**

1. 使用**支持 USB-PD 快充协议**、输出功率充足的电源适配器和充电线接入辅助 USB-C 接口（闪电图标），确保能同时为被控设备和 NanoKVM Go 供电。
2. 不要使用仅支持普通慢充或规格不明的充电线。
3. 如果不连接辅助 USB-C 接口，请确保被控设备的 USB-C 接口能稳定为 NanoKVM Go 供电。

> 提示：请务必使用合格、支持 PD 快充且功率充足的电源适配器和充电线，避免因供电不足导致设备反复重启或工作异常。
